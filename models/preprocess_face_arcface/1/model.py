...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        image_config = pb_utils.get_output_config_by_name(
                model_config, "x"
                )

        self.image_dtype = pb_utils.triton_string_to_numpy(
                image_config["data_type"]
                )
    ...
    def represent(
        ...
        boxes = [[x["x"], x["y"], x["w"], x["h"]] for x in batch_regions]

        return batch_images,boxes 
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            batch_images,boxes = self.represent(frame)    
            if batch_images is None:
                responses.append(pb_utils.InferenceResponse(
                                    error=pb_utils.TritonError("Face Not Detected")))
            else:

                #embeddings = self.get_batch_encodings(frame, face_locations)

                image_tensors = pb_utils.Tensor(
                            "x", np.array(batch_images).astype(self.image_dtype)
                    )
